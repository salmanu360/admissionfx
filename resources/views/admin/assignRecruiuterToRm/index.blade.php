...
<div class="page-header">
    <div class="page-title">
        <h3>All Assign Recruiter to rm</h3>
    </div>
    <div class="dropdown filter custom-dropdown-icon">
        <a><button class="btn btn-success" data-toggle="modal"data-target="#eduactionModel"><i class="fa-solid fa-square-plus"></i> Assign Recruiter</button></a>
    </div>
</div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-centers">
                                ...
                                data-toggle="modal"
                                data-target="#edieduactionModal{{$assign->id}}"
                                data-toggle="tooltip" data-placement="top"
                                title="edit eduaction">
                                <i class="fa fa-edit" aria-hidden="true"></i>
                            </button>
                            <a class="text text-danger btn-sm deleteRecord" style="color: white" href="javascript:void(0);" rel="{{ $assign->id }}" rel1="delete_recruiter" onclick="deleterecord('{{$assign->id}}','assign/destroy');">
                   ...
                   <form action="{{route('recruiterToRm.create')}}" method="POST">
                    ...
                    <div class="form-group">
                        <label for="recipient-name" class="form-control-label">Recruiter :</label>
                            <select class="form-control" name="recruiter[]" multiple>
                                @foreach ($recruiters as $recruiter)
                                    <option value="{{$recruiter->id}}:{{$recruiter->company_name}}">{{$recruiter->company_name}}</option>
                                @endforeach
                            </select>
                            <span style="color: rgb(82, 82, 187)">Help text:
                        <span>press Ctrl+mouse left button</span></span>
                            @error('recruiter')
